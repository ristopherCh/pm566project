...
drugs <- drugs[, .(state_name, state, indicator, data_value, date, population, state_number)]
...
drugs_usa <- drugs[, sum(data_value, na.rm = TRUE), by=list(indicator, date)]
setnames(drugs_usa, "V1", "data_value")
drugs_usa <- drugs_usa[order(date, indicator)]

# Creating drugs_usa2 for plotting
drugs_usa2 <- drugs_usa[indicator != c("Number of Deaths", "Number of Drug Overdose Deaths")]
drugs_usa2 <- drugs_usa2[indicator != "Percent with drugs specified"]

ggplot(drugs_usa2, aes(x = date, y = data_value, color = indicator)) +
  geom_line()
```
...
ggplot(drugs_usa2_2018, aes(x = date, y = data_value, color = indicator)) +
  geom_line()

indicators3 <- c(indicators[3],indicators[7],indicators[9])
drugs_usa3_2018 <- drugs_usa_2018[indicator %in% indicators3]

ggplot(drugs_usa3_2018, aes(x = date, y = data_value, color = indicator)) +
  geom_line(size = 1) +
  ...
  labs(x = "Date", y = "Data_Value", title = "Words") +
...
# This is the % increase in OD deaths from January 2020 to March 2021 per state-
od_012020 <- od_deaths[date == "2020-01-01"]
od_dif012020 <- (od_max$deaths_monthly - od_012020$deaths_monthly) * 100 / od_012020$deaths_monthly
od_012020$perc_inc <- od_dif012020
od_perc_incr012020 <- od_012020[, .(state_name, state, indicator, perc_inc)]
