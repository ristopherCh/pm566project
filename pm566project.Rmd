...
```{r}
ggplotly(ccpp) %>% layout(title = "COVID-19 cases by % of state population", font = list(size = 18))
```
...
![words](data/covmap.png)

![words2](data/odmap.png)
...
w1 <- ggplot(covid_od_econ, aes(x = econdrop, y = odincrease_apr20apr21, label = state_name, color = case_perc)) +
  geom_point() +
  geom_smooth() +
  labs(title = "Economic recession vs 12 month OD increase", x = "% drop in GDP, QTR4 2019 - QTR2 2020", y = "% increase in deaths by OD, 04/19-04/20 vs 04/20-04/21", color = "Cases of\nCOVID-19 by\n% state population")
