...
# The odpop_perc column is the percentage of the total state population that died of OD's from 04-20 to 04-21
...
od_cov1 <- ggplot(covid_od, aes(x = death_perc, y = odincrease_apr20apr21)) +
  geom_point() +
  geom_smooth() +
...
plot_geo() %>%
  add_trace(
    z = ~covid_od$case_perc, text = covid_od$state_name, span = I(0),
...
econ3 <- econ2 %>% filter(!GeoName %in% c("CALIFORNIA", "TEXAS", "NEW YORK", "FLORIDA"))
econ_min <- econ2[TimePeriod == "2020-01-01"]
setnames(econ_min, "DataValue", "relative_value")
econ4 <- merge(econ, econ_min, by = "GeoName")
econ4$relative <- econ4$DataValue / econ4$`relative_value`
...
```{r}
# drop
 # <- econ2$DataValue where TimePeriod = "2020-04-01" / econ2#DataValue where TimePeriod = "2020-01-01" by State
q1 <- econ2 %>% filter(TimePeriod == "2020-10-01")
q2 <- econ2 %>% filter(TimePeriod == "2020-01-01")
q3 <- 1 - (q1$DataValue / q2$DataValue)



econ5 <- econ2 %>% filter(TimePeriod == "2021-04-01")
econ5$econdrop <- q3
econ5 <- select(econ5, c("GeoName", "econdrop"))
q4 <- merge(covid_od, econ5, by.x = "state_name", by.y = "GeoName")
q5 <- merge(covid_od, econ5, by.x = "state_name", by.y = "GeoName")
#q4 <- q4 %>% filter(!state_name %in% c("ALASKA", "NEVADA", "WYOMING", "TENNESSEE"))
q4 <- q4 %>% filter(!state_name %in% c("NEVADA","TENNESSEE"))
...
w1 <- ggplot(q4, aes(x = econdrop, y = odincrease_apr20apr21)) +
  geom_point() +
  geom_smooth() +
...
ggplot(q4, aes(x = econdrop, y = death_perc)) +
  geom_point() +
  geom_smooth() +
  labs(title = "Economic recession vs 12 month OD increase")
...
```{r}
unem <- ggplot(unemployment, aes(x = date, y = value, color = STATE)) +
  geom_line()
...
```{r}
u0901 <- unemployment %>% filter(date == "2021-03-01")
u0301 <- unemployment %>% filter(date == "2020-03-01")
unemployment_change <- select(u0301, c("STATE"))
unemployment_change$unemp_change <- u0901$value - u0301$value
...
```{r unemployment_merge}
u2 <- merge(covid_od, unemployment_change, by.x = "state_name", by.y = "STATE")
#u2 <- u2 %>% filter(!state_name %in% c("HAWAII", "NEVADA", "MICHIGAN", "WYOMING"))
u2 <- u2 %>% filter(!state_name %in% c("HAWAII"))
```

```{r}
ggplot(u2, aes(x = unemp_change, y = odincrease_apr20apr21)) +
  geom_point() +
  geom_smooth() +
  labs(title = "Unemployment growth vs 12 month OD increase")
