...
gdu <- ggplot(drugs_usa, aes(x = date, y = deaths, color = indicator)) +
  ...
  labs(x = "Date", y = "12 month rolling average deaths from OD") +
  scale_x_date(date_breaks = "year",  date_labels = "%Y")

ggplotly(gdu) %>%
  layout(title = list(text = paste0("Total US OD deaths from all drug categories, 2015-2021",
                                    '<br>',
                                    '<sup>',
...
## Overdose deaths, by state {.tabset}
...
There are two ways of probing this question with graphs:
...
## Overdose deaths vs COVID-19 {.tabset}
...
## Geographical distributions {.tabset}
...
unemployment <- fread("stateunemployment.csv")
...
```{r}
u5 <- ggplot(covid_od_unemp, aes(x = umax_value, y = odpop_perc, label = state_name)) +
  geom_point() +
  geom_smooth() +
